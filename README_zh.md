...
纯 PyTorch 实现——不依赖 TileLang、DeepGEMM、vLLM。可以在 280B 模型的任意层插 hook，提取 4 路 Hyper-Connection 残差流、注意力输出、MoE 路由决策等中间状态。
...
我们用纯 PyTorch 替换了所有 GPU kernel：

| 原始实现（TileLang） | 我们的替换 | 说明 |
|---|---|---|
| `act_quant` | PyTorch absmax→scale→clamp→cast | 逐块 FP8 量化 |
| `fp4_act_quant` | PyTorch FP4 查表模拟量化 | 逐块 FP4 量化 |
| `fp8_gemm` | PyTorch 反量化→BF16→`torch.mm` | FP8×FP8 矩阵乘法 |
| `fp4_gemm` | int8→FP4_TABLE 手动解包→BF16→matmul | `float4_e2m1fn_x2.to()` 在 sm_121 上不可用 |
| `sparse_attn` | PyTorch for 循环 + bmm + softmax | 稀疏注意力 |
| `hc_split_sinkhorn` | PyTorch sigmoid + softmax + Sinkhorn 迭代 | Hyper-Connection 混合 |
...
```
Prefill: 5.5 秒（12 个 token）
生成：29 个 token / 52.7 秒（约 1.8 秒/token）
输出："你好！我是DeepSeek，一个由深度求索公司创造的AI助手，乐于为你解答问题、提供帮助和分享知识。"

激活值提取（第 28 层）：
  Shape: [1, 12, 4, 4096]（batch, 序列长度, 4 路 HC 残差流, 隐藏维度）
  4 路 norm: [133.00, 166.00, 113.50, 26.88]  ← 高度不对称
```

速度慢（纯 PyTorch，无 Triton 优化），但数值正确——输出与 vLLM 推理服务完全一致。做实验够用：提取激活值只需要跑一次 forward pass。
...
| 文件 | 说明 |
|------|------|
| `kernel_sm121.py` | 6 个 TileLang kernel 的纯 PyTorch 替换实现 |
